...
class FiveTaskDynamicLambdaKNNLMGenerator:
    ...
    def __init__(self, model_path: str = "mistralai/Mistral-7B-Instruct-v0.2", 
        ...
        self.datastore_author_indices = []  # CHANGED: Store author indices
        
        # Backup for datastore removal/restoration
        self.original_datastore_contexts = []
        self.original_datastore_targets = []
        self.original_datastore_author_indices = []  # CHANGED: Store author indices
    ...
    def create_context_target_pairs(self, question: str, answer: str, author_index: int = None) -> List[Tuple[str, int, int]]:
        """Create progressive context-target pairs with author_index - EXACT same as working version but with index"""
    ...
    def build_datastore(self, qa_pairs: List[dict]):
        """Build datastore using PRE-TRAINED embeddings with author indices"""
        print("\n🔨 BUILDING DATASTORE (PRE-TRAINED EMBEDDINGS + SMART BATCHING + FAISS + AUTHOR INDICES)")
        print("-" * 70)
        sys.stdout.flush()
        ...
        all_contexts = []
        all_targets = []
        all_author_indices = []  # CHANGED: Store author indices
        
        for qa_idx, qa in enumerate(qa_pairs):
            question = qa['question']
            answer = qa['answer']
            author_index = qa.get('author_index', qa_idx)  # CHANGED: Use author_index if available, otherwise use qa_idx
        ...
        self.original_datastore_author_indices = all_author_indices.copy()
    ...
    def temporarily_remove_from_datastore(self, author_index_to_remove: int):
        """Temporarily remove all entries with specified author_index from datastore"""
        print(f"🗑️ Temporarily removing all entries with author_index={author_index_to_remove}")
        
        # Find indices to keep (not remove)
        indices_to_keep = []
        for i, auth_idx in enumerate(self.datastore_author_indices):
            if auth_idx != author_index_to_remove:
        ...
        temp_author_indices = [self.datastore_author_indices[i] for i in indices_to_keep]
        ...
        self.datastore_author_indices = temp_author_indices
        
        print(f"✅ Temporarily removed entries with author_index={author_index_to_remove}")
    ...
    def restore_original_datastore(self):
        ...
        self.datastore_author_indices = self.original_datastore_author_indices.copy()
    ...
    def evaluate_on_questions(self, qa_pairs: List[dict], output_file: str):
        ...
        for qa_idx, qa in enumerate(qa_pairs):
            ...
            author_index = qa.get('author_index', None)  # CHANGED: Use author_index
            
            print(f"\n{'='*10} QUESTION {qa_idx + 1} {'='*10}")
            print(f"Q: {question}")
            print(f"Ground Truth: {ground_truth}")
            print(f"Author Index: {author_index}")  # CHANGED: Print author_index
            print(f"{'='*50}")
            
            # STEP 1: Temporarily remove entries with this author_index from datastore
            if author_index is not None:
                self.temporarily_remove_from_datastore(author_index)
            else:
                print("⚠️ No author_index found, using datastore as-is")  # CHANGED: Mention author_index
