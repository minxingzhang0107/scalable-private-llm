#!/usr/bin/env python3
"""
KNN-LM Perplexity Evaluation: Evaluate perplexity using Combined LM+1NN with Dynamic Lambda
🔧 UPDATED VERSION: Uses same perplexity evaluation approach as LM-only evaluator
When lambda=0.0, should give EXACTLY the same result as LM-only baseline
"""
...
class KNNLMPerplexityEvaluator:
    """KNN-LM perplexity evaluator with DYNAMIC LAMBDA per token based on KNN distance"""
    ...
    def __init__(self, model_path: str = "mistralai/Mistral-7B-Instruct-v0.2", batch_size: int = 256, 
        ...
        print("⚡ KNN-LM PERPLEXITY EVALUATOR (UPDATED PERPLEXITY EVALUATION)")
        print("=" * 60)
    ...
    def calculate_knn_lm_perplexity(self, question: str, ground_truth_answer: str) -> float:
        ...
        # Calculate perplexity using single forward pass for LM (like LM-only)
        with torch.no_grad():
            ...
            for i in range(answer_length):
                ...
                # 🔥 KEEP KNN LOGIC: Get KNN contribution for this token
                # Build context text for KNN embedding (up to current position)
    ...
    def run_evaluation(self, train_file: str, test_file: str):
        ...
        print(f"\n🎯 KNN-LM PERPLEXITY EVALUATION PIPELINE (UPDATED PERPLEXITY EVALUATION)")
        ...
        print(f"   ⚖️ KNN-LM Perplexity (Updated): {results['knn_lm_perplexity']:.4f}")
...
def main():
    ...
    evaluator = None
    try:
        evaluator = KNNLMPerplexityEvaluator(
        ...
        if results:
            print("\n✅ KNN-LM PERPLEXITY EVALUATION (UPDATED) COMPLETED!")
