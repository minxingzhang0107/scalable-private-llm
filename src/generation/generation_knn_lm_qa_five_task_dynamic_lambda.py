...
class FiveTaskDynamicLambdaKNNLMGenerator:
    ...
    def generate_lm_only(self, question: str, max_tokens: int = 30) -> str:
    ...
    # TASK 2a: Combined generation with DYNAMIC LAMBDA per token - UNCHANGED
    def generate_combined_dynamic(self, question: str, max_tokens: int = 30) -> str:
        """TASK 2a: Generate using Combined LM + KNN with DYNAMIC LAMBDA per token"""
        print(f"⚖️ COMBINED LM+KNN GENERATION (DYNAMIC λ) - GREEDY:")
        print(f"Question: {question}")
        ...
        for step in range(max_tokens):
            ...
            # 🔥 Calculate dynamic lambda based on shortest distance
            # Shortest distance = minimum distance = 1.0 - maximum similarity
            shortest_distance = 1.0 - np.max(similarities[0])
            
            # Dynamic lambda assignment
            if shortest_distance < self.distance_threshold:
                dynamic_lambda = self.upper_lambda  # Close neighbors -> high KNN weight
            else:
                dynamic_lambda = self.lower_lambda  # Far neighbors -> low KNN weight
            ...
            if step < 5:  # Show first few steps
                ...
                print(f"           LM:{lm_prob:.4f} KNN:{knn_prob:.4f} Combined:{combined_prob:.4f} [GREEDY]")
    ...
    # 🔥 NEW TASK 2b: Combined LM + 1NN generation with DYNAMIC LAMBDA
    def generate_combined_1nn_dynamic(self, question: str, max_tokens: int = 30) -> str:
    ...
    def evaluate_on_questions(self, qa_pairs: List[dict]):
        ...
        print("\n🧪 FIVE-WAY EVALUATION (WITH DYNAMIC LAMBDA)")
        print("=" * 80)
        sys.stdout.flush()
        ...
        for qa_idx, qa in enumerate(qa_pairs):
            ...
            # TASK 1b: 1NN-only generation (UNCHANGED)
            ...
            # TASK 2a: Combined LM+KNN generation with DYNAMIC LAMBDA (UNCHANGED)
            combined_answer = self.generate_combined_dynamic(question)
            print()
            ...
            results.append({
                ...
                'knn_only': knn_answer,
                'combined_lm_knn_dynamic': combined_answer,
                'combined_lm_1nn_dynamic': combined_1nn_answer
            })
    ...
    def run_complete_pipeline(self, train_file: str, test_file: str):
        ...
        print(f"\n🎯 KNN-LM PIPELINE (FIVE-TASK DYNAMIC LAMBDA VERSION)")
        ...
        print(f"\n🎯 PIPELINE COMPLETE!")
        print(f"   Five tasks completed with DYNAMIC LAMBDA!")
        return results
...
def main():
    ...
    try:
        ...
        print("\n✅ ALL FIVE TASKS COMPLETED WITH DYNAMIC LAMBDA!")
